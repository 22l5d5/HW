...
### 5.1 程式碼是否符合作業規範
- **ADT 與私有成員**：`Node` 包含 `coef`、`exp` 和 `link`，`Polynomial` 使用循環鏈表與頭節點，符合要求。
- **輸入輸出**：`operator>>` 支援指定項數輸入，`operator<<` 輸出外部表示，符合 PDF 規範。
- **功能實現**：所有指定函數（a-i）實現完整，支援負指數。
- **限制與改進**：
  - `mergeTerms` 時間複雜度為 \( O(n^2) \)，可優化為 \( O(n \log n) \) 使用排序。
  - 假設輸入指數遞減，未來可加入排序驗證。
  - 可用空間鏈表實現簡單，實際應用中可擴展為記憶池。
