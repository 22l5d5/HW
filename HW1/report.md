...
程式能正確計算用戶輸入的 ( m ) 和 ( n ) 對應的 Ackermann 函數值（限於 ( m = 0, 1, 2, 3 )），使用非遞迴方法。

測試案例涵蓋不同 ( m ) 值的情況，驗證了程式的正確性。

## 申論及開發報告

### 選擇方法的理由

非遞迴方法：符合題目要求，避免遞迴呼叫，通過條件語句和簡單迭代實現，適合有限範圍內的計算。

簡化實現：針對 ( m = 0 ) 到 ( m = 3 ) 的特例設計公式，減少計算複雜度。
